...
const AdminPortfolio = () => {
  ...
  const save = async () => {
    ...
    const res = await api.post(endpoint, payload as any).catch(async () => {
      // Fallback using fetch when the ApiClient route is not set up
      const r = await fetch(`/api${endpoint}`, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
